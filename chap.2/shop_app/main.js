...
items.addEventListener('click', event => {
                    // if (event.target.nodeName === 'I') { 대신 dataset.id활용

    const id = event.target.dataset.id; // 반복해서 쓰일 거라 id에 할당해줌
    if (event.target.dataset.id) {              //받아온 id 써야하니까 ` `스트링으로 변경
        const toBeDeleted = document.querySelector(`.item__row[data-id="${id}"]`); //클래스명이 기억 안날 땐 console창에 document.quertySelector('item[data-id="0"]'); 활용해보기(휴지통 버튼 떠있을때)
        toBeDeleted.remove();
    };
});
